...
const Page: React.FC = () => {
  ...
  const [coverImagePublicUrl, setCoverImagePublicUrl] = useState<string | undefined>();
  const [coverImageKey, setCoverImageKey] = useState<string | undefined>();
  const { session } = useAuth();

  const handleImageChange = async (e: ChangeEvent<HTMLInputElement>) => {
    setCoverImageKey(undefined); // 画像のキーをリセット
    setCoverImagePublicUrl(undefined); // 画像のURLをリセット
    ...
    // 画像のURLを取得
    setCoverImagePublicUrl(publicUrlResult.data.publicUrl);
  };
  ...
  return (
    <div>
      ...
      <div className="text-sm break-all">coverImagePublicUrl : {coverImagePublicUrl}</div>
    </div>
  );
};
