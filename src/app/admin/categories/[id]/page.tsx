...
const Page: React.FC = () => {
  ...
  return (
    <main className="px-4 py-6 max-w-4xl mx-auto">
      <div className="mb-6">
        <div className="text-2xl sm:text-3xl font-bold">カテゴリの編集・削除</div>
      </div>

      <form
        onSubmit={handleSubmit}
        className={twMerge("mb-4 space-y-4 sm:space-y-5", isSubmitting && "opacity-50")}
      >
        <div className="space-y-6">
          <div className="space-y-1">
            <div className="block font-bold text-sm sm:text-base">現在のカテゴリの名前</div>
            <div className="text-gray-500 text-sm sm:text-base">{currentCategoryName}</div>
          </div>

          <div className="space-y-2">
            <label htmlFor="name" className="block font-bold text-sm sm:text-base">
              新しいカテゴリの名前
            </label>
            <input
              type="text"
            ...
            {newCategoryNameError && (
              <div className="flex items-center space-x-1 text-xs sm:text-sm font-bold text-red-500">
                <FontAwesomeIcon
      ...
      <div className="mb-2 text-2xl sm:text-3xl font-bold">既存のカテゴリの一覧</div>
      {categories.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((category) => (
              <div
                key={category.id}
                className={twMerge(
                  "rounded-md px-2 py-0.5 text-sm sm:text-base",
